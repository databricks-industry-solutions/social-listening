...
Note that if you want to have multiple demo apps in the same Databricks workspace, make sure to update at least the bundle name in `databricks.yml`, and change or remove the bundle UUID to prevent subsequent deploys from overwriting each other. If you still encounter errors, try deleting the `.databricks` directory (autogenerated by DAB validation/deployment) and retrying.
