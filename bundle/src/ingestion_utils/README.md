...
To remove sampling from the Steam ingestion, call the `SteamIngestor::ingest()` function with the argument `sampling=False`. To get all available reviews, additionally set `num_reviews=None` or `num_reviews=0`.
...
- The class may be found at `../bundle/src/ingestion_utils/reddit_ingestor.py`, and the function call to change is the ingest() call in the `../Abstracted_Ingestion` notebook.
